...
- [x] `2026-09-15` Complete remaining Melodify Studio + UI checkpoints on `mimi`: expanded authenticated `/api/karaoke/discovery` using catalog/provider foundations with bounded query/page/limit parsing, Bangla/Kolkata/Hindi/English region filters, deterministic dedupe, and `KARAOKE_READY`/`SING_ALONG` classification; fixed recording start sequence so selected backing starts deterministically with mic capture and safe cleanup; added direct Web Audio mix path for local-ready tracks (backing to speakers + recorder stream, mic to recorder only) with MIME negotiation and backing/vocal gain controls; added synchronized composite mode for provider tracks (mic-only capture plus persisted sync metadata and preview play/pause/seek synchronization without provider audio capture); modernized Studio upload/progress status card with accessible progress semantics; themed Dashboard section scrollbars with scoped thin dark/cyan styling (no global html/body scrollbar overrides); added focused regressions across Studio discovery/flow/mix/composite/upload and Dashboard scrollbars plus server parsing/recording metadata tests.

- [x] `2026-09-15` Add bounded lyrics prefetch CLI on `mimi`: created `server/scripts/prefetchLyrics.js` with strict args (`--limit`, `--offset`, `--dry-run`, `--include-legacy`), filtered candidate selection, provider-resolution prefetch flow, safe metadata persistence (`lyrics_source/match/provider_id/checked_at` + provider plain lyrics), and deterministic JSON reporting; added tests in `server/scripts/prefetchLyrics.test.js`; exposed `npm run lyrics:prefetch`; updated README usage.
