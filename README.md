...
- **Dashboard Trending Now (22/43)** — Dashboard still loads one authenticated `GET /api/trending?limit=10` per mount via the existing API client; activity and catalog-fallback cards stay semantically distinct with **no numeric Trending score** and **no fabricated fallback activity metrics**; playback passes the full visible Song list to `player.playSong(list, index)` for next/previous continuity; server **503** quietly hides the section; empty/error states stay section-local; PlayHistory/listening events remain centralized in PlayerContext (no direct Dashboard telemetry writes)
...
- **Immutable evaluation-run persistence (32/43)** — `server/models/RecommendationEvaluationRun.js` + `server/services/recommendationEvaluationRunService.js` append-only Mongo history for 28/43 evaluation results: schema version 1; caller-supplied safe run IDs (`^[a-z0-9][a-z0-9._-]{0,63}$`, unique index); stages `collaborative|hybrid|policy`; optional artifact version; required `evaluated_at` (ISO-8601 with timezone, distinct from DB `createdAt`); ten 28/43 metrics each in [0,1] **separately** with **no overall/combined quality score**; factual summary and dataset split counts (conservation, no silent correction); bounded configuration (seed/factor counts/weight pairs/exploration interval, no silent renormalization); service-computed deterministic SHA-256 over the normalized payload for idempotent retry identity (**not** an auth signature); identical retry → same record, same run_id different content → conflict; documents are **immutable** (no update/replace/delete, no TTL); newest-first listing by `evaluated_at` then `_id`; **no** raw user IDs, recommendation lists, ListeningEvents, Favorite/Playlist rows, emails, tokens, or IPs persisted; no Python execution, training, ranking, snapshot generation, routes, or Dashboard/Admin UI — **33/43** will add deterministic end-to-end recommender smoke tests
- **Deterministic recommender smoke coverage (33/43)** — test-only end-to-end composition of the existing Python APIs, with train-only temporal evidence, held-out/new-Song and cold-start exploration coverage, evaluation wiring, full-run repeatability, and input immutability checks; see the smoke-test section below.
- **Immutable recommendation snapshots (34/43)** — `server/models/RecommendationSnapshot.js` + `server/services/recommendationSnapshotService.js` persist already-ranked final 31/43 policy output in MongoDB without running Python, loading SVD, or recomputing hybrid/cold-start scores at request time; schema version 1; each snapshot belongs to exactly one internal User (`user_id` → User ObjectId) and one caller-supplied `snapshot_version` (`^[a-z0-9][a-z0-9._-]{0,63}$`); uniqueness is per **user + snapshot_version** (the same version may exist once per user, not globally); required caller-supplied `generated_at` (ISO-8601 with explicit timezone or a Date — never `Date.now()`), distinct from DB `createdAt`; optional `artifact_version` (same safe identifier policy or null) may later link to a 27/43 release without requiring that release to exist; ordered items store rank, Song ObjectId reference only (no title/artist/thumbnail/youtube_id duplication — 35/43 loads current Song metadata), basis, `policy_score`/`hybrid_score`/`profile_score` (null or finite [0,1], not probabilities/confidence), and `collaborative_known`; exact basis vocabulary is `hybrid`, `hybrid-profile`, `profile`, `exploration`; exploration items persist `policy_score=null` rather than a fabricated ranking-quality score; the snapshot layer validates structure, ranges, contiguity (ranks exactly 1..N), duplicate Song rejection, basis↔score↔collaborative_known semantics, and 31/43 summary conservation invariants — it does **not** recompute 0.80/0.20 or 0.70/0.30 policy formulas; empty snapshots (items `[]`) are valid and supersede older non-empty snapshots via latest `generated_at` ordering; normalized payload receives a deterministic service-computed SHA-256 `payload_sha256` (excludes `_id`/`createdAt`/`updatedAt`/`payload_sha256`) for idempotent retry identity — **not** auth/signature/quality; identical retry for the same user/version → `created:false`; changed content for the same user/version → `RecommendationSnapshotConflictError` (no update/overwrite/merge); published documents are immutable history (no update/replace/delete, no TTL); latest lookup is `generated_at DESC, _id DESC` with `.lean()`; raw ListeningEvents, Favorite/Playlist contents, explicit profile counts, emails, credentials, and IPs are **not** copied into snapshots; no route, `server.js`, auth, UI, or Python logic was added — **35/43** will add the secure authenticated personalized recommendation API reading the latest snapshot.
- **Personalized recommendation API (35/43)** — authenticated `GET /api/recommendations` (`server/routes/recommendationRoutes.js` + `server/services/personalizedRecommendationService.js`); **flag** is exactly `RECOMMENDATION_AI_ENABLED` / `recommendationConfig.aiEnabled` (fixed 503 when disabled, zero reads); **playability rule** is non-empty trimmed `youtube_id` OR `file_path`, non-empty `title`, and `recommendation_eligible !== false` (absent stays eligible); **public Song fields** are `_id`, `title`, `artist`, `genre`, `youtube_id`, `file_path`, `poster_url`, `duration` + optional `duration_seconds`, `release_date`, `language`, `category` — never `recommendation_eligible`/basis/scores/artifact; **37/43** Dashboard integration.
- **Personalized recommendation client states (36/43)** — reusable client layer without Dashboard rendering changes: `client/src/services/personalizedRecommendations.js` + `client/src/hooks/usePersonalizedRecommendations.js` + `client/src/pages/Dashboard/recommendationUi.js`; exact states `idle | loading | ready | no-snapshot | empty | disabled | error`; one `GET /api/recommendations?limit=<1..100>` per fetch (default 10, no userId/email/snapshot params, no auto-retry/polling); `ready` requires `status==="ready"` with items, `empty` is `status==="ready"` with zero items (never coerced to `no-snapshot`), `disabled` only on the exact checkpoint-35 disabled message, `error` for every other failure; server order preserved (no client re-rank/random/popularity); legacy fallback helper is false for `idle|loading|ready` and true for `no-snapshot|empty|disabled|error`.
- **Dashboard Recommended For You (37/43)** — Dashboard consumes the reusable 36/43 `usePersonalizedRecommendations({limit:10})` hook; `ready` displays only personalized snapshot Songs in server/persisted order (no client append/interleave/rerank); `loading`/`idle` show a neutral placeholder and do **not** flash legacy cards; `no-snapshot`, `empty`, `disabled`, and `error` preserve legacy fallback songs with **no new fallback request** (not coupled to Trending); clicking a recommendation passes the exact visible list and index to `PlayerContext.playSong(...)`; no manual history/listening telemetry write; snapshot diagnostic scores/internal metadata are not rendered; no server, Python, API contract, model, or dependency changes.
...
- **Admin evaluation history (42/43)** — authenticated read-only **`GET /api/admin/recommendations/history`** behind the existing `protect` then `adminOnly` middleware (same mount as metrics; no feature flag or custom JWT/role parsing); optional query keys only `pipeline_stage` (exact lowercase, default `policy`) and `limit` (`1..100`, default `20`, strict integer-string validation with no clamping); one `listEvaluationRuns({limit, pipeline_stage})` call per request; response envelope `{state, source, pipeline_stage, limit, count, runs}` with `state` exactly `ready|no-runs` and `source` `evaluation-history`; each run exposes only the whitelist fields (`run_id`, `pipeline_stage`, `artifact_version`, `evaluated_at`, ten metrics, seven summary counts + invariants, nine dataset counts + conservation, nine configuration values with missing optionals as `null`) — no `payload_sha256`/`schema_version`/`_id`/timestamps, no user/song/event data, no re-sorting or quality ranking (server order preserved); the Admin page gains **Evaluation History** (row list with local selection via `aria-pressed`), **Dataset Statistics**, and **Reproducibility Configuration** sections while keeping the 41/43 latest-metrics dashboard intact; shared single stage selector; client service + hook + pure UI helper; no Train/Retrain/Evaluate/Activate buttons, no charts, no overall/winner/best labels, no second stage selector; server, models, middleware, `server.js`, `ml/`, Dashboard, and dependencies unchanged.
- **Bounded offline retraining + Admin Model Health (43/43)** — final numbered checkpoint; retraining is triggered **only** by the offline CLI (`node server/scripts/retrainRecommendations.js --run-id <safe-run-id> --run-at <timezone-aware-ISO8601> [--snapshot-limit <1-100>]`) — **no HTTP endpoint can start a retrain/evaluate/activate run**; one bounded run collects ≤250,000 events / 50,000 users / 25,000 songs (seed 42), spawns `python -m ml.recommender.cli retrain-json` exactly once (`spawn` with `shell:false`, 600,000 ms timeout, 256 MiB stdin/stdout caps — the only `child_process` use in the server), then persists the 38/43 evaluation run + 35/43 snapshots under the shared identity `evaluation run_id = snapshot_version = artifact_version = run_id`, with lease-based single-flight and exactly ten failure codes; the Admin page gains a read-only **Model Health** section fed by authenticated **`GET /api/admin/recommendations/health`** (`protect` → `adminOnly`, no query parameters allowed, states `running|never-run|completed|failed`, lease projected to `{active, run_id, expires_at}` without any token, latest attempt as a fail-closed whitelist) — one GET per mount plus explicit **Refresh Status**, no polling, **no health score/winner/quality label**, no retrain button, no `RECOMMENDATION_AI_ENABLED` gate; see the dedicated section below.
- **Global + full-screen audio player surfaces** — persistent desktop player bar and mobile mini-player across shell routes now include a non-fullscreen Lyrics/Chords drawer, while full-screen uses a split control+lyrics layout with safe-area handling, engine-confirmed playback status messaging, retry action, and complete transport controls
- **Lyrics/chords data authority + editing flow** — `GET /api/lyrics/:songId` remains lyrics-first from Song data with LRCLIB fallback, now always includes persisted `chords`; `LyricsChordsPanel` hydrates chords from that authoritative payload (not from queue shape), and Admin Music edit can update both `lyrics` and `chords` through `/api/songs/:id/content`
...
- Music catalog management (add/edit songs, including lyrics/chords)
...
│   ├── services/recommendationEvaluationRunService.js # Immutable evaluation-run persistence (32/43)
│   ├── services/recommendationSnapshotService.js # Immutable recommendation snapshot persistence (34/43)
...
│   ├── routes/                    # /api/auth, /api/songs, /api/lyrics, /api/playlists, /api/history, /api/subscriptions, /api/admin, /api/listening-events, /api/trending, /api/recommendations, /api/admin/recommendations/metrics
...
│   └── tests/                      # unittest suites including test_recommender_pipeline_smoke.py (33/43)
...
| Timeout | **No in-process training timeout** in 23/43 itself; the 43/43 Node runner enforces a **600,000 ms** subprocess wall-clock limit (`RETRAIN_PYTHON_TIMEOUT_MS`) around `retrain-json` |
...
| CLI | argparse only — `runtime-info` (optional `--json`) plus `retrain-json` (added in 43/43, reads one bounded stdin JSON document); **no** `train` / `evaluate` / `recommend` / `snapshot` / `serve` commands |
...
| Not present | snapshot generation/orchestration, authenticated recommendation API/UI (35+/43), retraining, HTTP/MongoDB/CLI ranking command, Dashboard UI changes |
| Next | **32/43** persists evaluation-run history (implemented below) |
...
### Immutable evaluation-run persistence (32/43)

`server/models/RecommendationEvaluationRun.js` and `server/services/recommendationEvaluationRunService.js` provide append-only Node/Mongo persistence for 28/43 evaluation metrics. This is a **storage contract only** — no HTTP route, no Python subprocess, no training, no ranking, no artifact publication, and no Admin/Dashboard UI in this checkpoint.

| Topic | Behavior |
|---|---|
| Schema version | `EVALUATION_RUN_SCHEMA_VERSION=1`; caller may omit (defaults to 1); explicit non-1 rejects |
| Run ID | caller-supplied trimmed `^[a-z0-9][a-z0-9._-]{0,63}$` (length 1..64); rejects `.`, `..`, `/`, `\`, `:`, spaces, uppercase; unique index |
| Pipeline stage | exactly `collaborative`, `hybrid`, or `policy` |
| Artifact version | optional / `null` / same identifier policy as 27/43 (`^[a-z0-9][a-z0-9._-]{0,63}$`, max 64) |
| `evaluated_at` | required caller-supplied Date or ISO-8601 **with explicit timezone** (rejects tz-less, date-only, numeric epoch, missing); never `Date.now()`; distinct from DB `createdAt` |
| Metrics | ten keys only — `precision_at_5/10`, `recall_at_5/10`, `ndcg_at_5/10`, `map_at_10`, `hit_rate_at_10`, `catalog_coverage`, `diversity`; each finite Number in [0,1]; **no** `overall_score` / `quality_score` / winner |
| Summary | 7 non-negative integers with `evaluated_user_count ≤ relevance_user_count`, `unique_recommended_at_10 ≤ catalog_size`, `diversity_evaluable_user_count ≤ evaluated_user_count`; `recommendation_user_count` may exceed evaluated |
| Dataset | 9 non-negative integer counts; conservation `raw == train + validation + test` (all-zero when raw==0); never silent correction |
| Configuration | `random_seed` required integer `0..4294967295`; optional `algorithm` identifier, `requested_components`/`effective_components` (1..32, effective ≤ requested), paired `collaborative_weight`+`content_weight` and `base_hybrid_policy_weight`+`explicit_profile_policy_weight` (each 0..1, pair sums ≈1 within 1e-9), `exploration_interval` (1..100); no silent renormalization |
| Strict keys | unknown keys rejected at top level, metrics, summary, dataset, and configuration |
| `payload_sha256` | **service-computed** lowercase 64-hex SHA-256 over the normalized payload (schema_version, run_id, pipeline_stage, artifact_version, `evaluated_at` as UTC ISO, metrics, summary, dataset, configuration; excludes `_id`/`createdAt`/`updatedAt`/`payload_sha256`); deterministic canonical key order; purpose = idempotent retry + immutable-content identity, **not** auth/encryption/signature |
| Record | one `create` → on duplicate-key code `11000` only, one bounded `findOne({run_id})`: same hash → `{created:false}`, different hash → `EvaluationRunConflictError`, missing → `EvaluationRunPersistenceError`; no upsert/update/replace/second-create loop |
| List | options only `limit` (default 20, max 100) and optional `pipeline_stage`; sort `evaluated_at:-1, _id:-1`; `.lean()`; no metric/best sort, no cursor pagination |
| Immutability | fields marked `immutable`; `pre('save')` rejects non-new documents; query guards on `updateOne`/`updateMany`/`findOneAndUpdate`/`replaceOne`/`deleteOne`/`deleteMany`/`findOneAndDelete`/`findOneAndRemove` + document `deleteOne`; `timestamps:true`, `versionKey:false`, `strict:true`, no `Mixed`, **no TTL** |
| Privacy | documents persist **aggregate metrics/counts only** — no user IDs, recommendation lists, raw ListeningEvents, Favorite/Playlist rows, user profiles, emails, tokens, or IPs |
| Errors | `EvaluationRunValidationError`, `EvaluationRunConflictError`, `EvaluationRunPersistenceError`, `EvaluationRunImmutableError` under `EvaluationRunError`; bounded messages |
| Not present | Admin/public route (38/43), Python execution/training/ranking/snapshot generation, best-model selection, Dashboard UI, raw event storage |
| Next | **34/43** persists immutable personalized recommendation snapshots (implemented below); **35/43** serves them via `GET /api/recommendations` (implemented below) |

```bash
node --test server/services/recommendationEvaluationRunService.test.js
```

### Deterministic end-to-end recommender smoke tests (33/43)

`ml/tests/test_recommender_pipeline_smoke.py` composes the existing public APIs:
raw records → temporal split → sparse interactions → Song content features →
collaborative TruncatedSVD → hybrid ranking → cold-start/exploration policy → evaluation.

- The tiny synthetic fixture has three temporal-history users, one collaborative-cold-start user, six Songs, and 27 fixed UTC events across nine sessions. U1 uses S1/S2/S4, U2 S2/S3/S1, and U3 S3/S1/S2 for train/validation/test respectively.
- Only `split.train` feeds the sparse builder; validation/test events are excluded. Held-out-only S4 is absent from the training bundle and collaborative model but present in the full content catalog. The 31/43 policy ranks it using explicit artist evidence without fabricating a collaborative score. Zero-feature S6 remains eligible through deterministic exploration.
- Relevance is grouped from `split.test`, without playback-event-type inference. Evaluation consumes actual ordered policy output. Exhausting each user's five eligible candidates gives stable precision/recall/hit/coverage invariants; rank-dependent metrics are checked against actual ranks, with no arbitrary quality threshold or production-quality claim.
- Complete pipeline reruns compare splits, sparse/content matrices and indexes, latent factors (tight `assert_allclose`), rankings, and evaluation. Input dictionaries, matrices, and model factors are also checked for mutation; U4 exploration is repeatable even with reversed candidates.
- The smoke flow uses in-memory synthetic data only: no database/network, filesystem artifact output, publication, subprocess, or new production orchestration. Checkpoint 33/43 adds no production recommender behavior; 32/43 persistence remains independently covered by backend tests.

```bash
python -m unittest ml.tests.test_recommender_pipeline_smoke
python -m unittest discover -s ml/tests -p "test_*.py"
```

### Immutable recommendation snapshots (34/43)

`server/models/RecommendationSnapshot.js` + `server/services/recommendationSnapshotService.js` store the **already-ranked** final 31/43 policy output so a future authenticated API can serve personalized recommendations without running Python, loading SVD, recomputing hybrid/cold-start scores, or querying raw listening history at request time.

- **Ownership & identity** — every snapshot belongs to exactly one internal Melodify User (`user` ObjectId → `User`). Callers supply `user_id` (canonical 24-hex) and `snapshot_version` (`^[a-z0-9][a-z0-9._-]{0,63}$`; lowercase only; rejects `.`/`..`, spaces, `/`, `\`, `:`, uppercase). The compound unique index is `{ user, snapshot_version }` — one user may keep many historical versions, and the **same** `snapshot_version` may exist once per different user (not globally unique).
- **Times & optional artifact** — required `generated_at` is caller-supplied generation time (ISO-8601 with explicit timezone, or a valid Date; never `Date.now()`), distinct from DB `createdAt`. Optional `artifact_version` is `null` or the same safe identifier policy and does not require the artifact to exist; 34/43 never touches `ml/artifacts/`.
- **Items (0..100)** — ordered array of `{ rank, song, basis, policy_score, hybrid_score, profile_score, collaborative_known }`. `song` is only an ObjectId reference (`song_id` on input); title/artist/thumbnail/youtube_id are **not** duplicated. Ranks must be exactly `1..N` with no gaps/duplicates/reordering. A Song ID appears at most once (canonical duplicates reject — no silent dedup). Exact basis vocabulary: `hybrid`, `hybrid-profile`, `profile`, `exploration`. Scores are `null` or finite `[0,1]` (not rounded; not confidence/probability/rating). Structural semantics: `hybrid` needs numeric policy+hybrid and null profile with `collaborative_known=true`; `hybrid-profile` needs all three numeric with `collaborative_known=true`; `profile` needs numeric policy+profile and null hybrid (collaborative_known may be either); `exploration` requires `policy_score=null` (hybrid/profile diagnostics optional) and a numeric hybrid diagnostic implies `collaborative_known=true`. The service validates structure only — it does **not** recompute 0.80/0.20 or 0.70/0.30 formulas.
- **Summary (13 fields)** — factual 31/43 counts (`input_candidate_count` … `profile_available`) with conservation invariants: profile+seen+eligible=input; known+cold-start=eligible; exploit+exploration=returned; returned=items.length; returned≤requested_limit (1..100); `profile_available` iff `profile_feature_count>0`; unknown collaborative user requires `seen_excluded_count=0`; exploration item count matches the summary. Empty snapshots (`items: []`) are **valid** and can supersede older non-empty snapshots via latest `generated_at` ordering — no silent fallback to stale recommendations.
- **Fingerprint & idempotency** — service-computed lowercase 64-hex SHA-256 `payload_sha256` over the normalized payload (`schema_version`, `user_id`, `snapshot_version`, `artifact_version`, UTC `generated_at`, ordered items, ordered summary; excludes `_id`/`createdAt`/`updatedAt`/`payload_sha256`) for idempotent retry identity — **not** encryption, authentication, signature, or quality. One create attempt: on E11000-only a single `findOne({user, snapshot_version})` — same hash → `created:false` + existing snapshot; different hash → `RecommendationSnapshotConflictError` (no update/replace/merge); missing → `RecommendationSnapshotPersistenceError`. Non-E11000 errors are sanitized with no retry loop.
- **Reads** — `getLatestRecommendationSnapshotForUser(userId)` sorts `generated_at DESC, _id DESC` and returns a lean snapshot or `null` (primary path for 35/43). `getRecommendationSnapshotByVersion(userId, snapshotVersion)` is an exact lean lookup or `null`. No global list-all API, no activation pointer, no User/Song existence queries, no N+1.
- **Immutability** — documents reject `save` after insert, `updateOne`/`updateMany`/`findOneAndUpdate`/`replaceOne`, and `deleteOne`/`deleteMany`/`findOneAndDelete`/`findOneAndRemove` (plus document delete). `timestamps:true`, `versionKey:false`, `strict:true`, explicit sub-schemas (no Mixed), **no TTL**. Four indexes: unique `{user, snapshot_version}`; `{user, generated_at, _id}`; `{snapshot_version, generated_at}`; partial `{artifact_version, generated_at}`.
- **Privacy** — stores only the internal user ObjectId, ordered Song ObjectIds, ranking diagnostics, and factual summary counts. Does **not** copy email, username, password, JWT/tokens, IP, raw ListeningEvents, Favorite/Playlist document contents, `artist_counts`/`genre_counts`, or search history.
- **Out of scope** — no route (`/api/recommendations*`), no `server.js`/auth/UI/Python changes, no training/ranking/evaluation/artifact publication, no scheduler. **35/43** will add the secure authenticated personalized recommendation API reading the latest snapshot.

```bash
node --test server/services/recommendationSnapshotService.test.js
```

### Personalized recommendation API (35/43)

`GET /api/recommendations` serves the caller's latest 34/43 snapshot with current Song metadata. Production router: `server/routes/recommendationRoutes.js`; loader: `server/services/personalizedRecommendationService.js`.

- **Auth & flag** — `protect` only (no `adminOnly`). Feature flag is exactly `RECOMMENDATION_AI_ENABLED` (accessor `recommendationConfig.aiEnabled` from `server/config/recommendation.js`). When disabled: fixed HTTP 503 `{ success:false, error:"Personalized recommendations are currently unavailable." }` with **zero** snapshot/Song reads (flag checked before parse and service).
- **Query** — only accepted key is `limit` (string digits, integer 1..100, default 10). Unknown keys, `0`/`101`/negative/float/`NaN`/empty/whitespace/array/boolean/non-string → HTTP 400 `Invalid recommendation query` before any service read.
- **Identity** — solely `req.user._id` from `protect`; never `req.query.user` / body / header tokens.
- **Reads** — exactly one `getLatestRecommendationSnapshotForUser` (reuse 34/43, no version fallback, no older-snapshot walk) then one bulk `Song.find({ _id: { $in } })` limited to ≤100 unique snapshot Song IDs (no N+1). Read-only: no create/update/delete, no PlayHistory/ListeningEvent writes, no Python/SVD/ranking at request time.
- **Availability filter (before limit)** — a snapshot item is available only when the Song document exists, `recommendation_eligible !== false` (absent/undefined legacy value stays eligible), playable (`youtube_id` OR `file_path` non-empty after trim — whitespace-only is not playable), and `title` is non-empty. Missing/deleted Songs count as unavailable and never invent placeholders.
- **Order & states** — snapshot order is authoritative (no re-score/re-rank/popularity/random/Trending fallback). Status is `ready` when a latest snapshot exists (including **empty** `items: []` → zero counts, still `ready`) or `no-snapshot` when none. Source is always `personalized-snapshot`.
- **Response envelope** — `{ success:true, data:{ status, source, items, snapshot } }`. Each item is `{ rank, song }` with contiguous ranks 1..N after filtering+limit. `snapshot` metadata only: `{ snapshot_version, generated_at, item_count, available_count, unavailable_count }` where `available_count + unavailable_count === item_count`.
- **Public Song fields** — `_id`, `title`, `artist`, `genre`, `youtube_id`, `file_path`, `poster_url`, `duration`, plus optional `duration_seconds`, `release_date`, `language`, `category` when present. Never expose `recommendation_eligible`, `basis`, `policy_score`, `hybrid_score`, `profile_score`, `collaborative_known`, `artifact_version`, `payload_sha256`, snapshot `_id`/`summary`, or user email/ObjectId beyond what the player needs.
- **HTTP map** — 503 disabled · 400 bad query/limit · 401 `protect` · 200 `ready`/`no-snapshot` · 500 unexpected with fixed `failed to load personalized recommendations` (no `error.message`/stack leak).
- **Out of scope** — Admin recommendation-metrics API is **38/43** (implemented below); snapshot writing, second feature flag, Python execution.

```bash
node --test server/services/personalizedRecommendationService.test.js server/routes/recommendationRoutes.test.js
```

### Personalized recommendation client states (36/43)

Reusable client consumption layer for `GET /api/recommendations`. Service: `client/src/services/personalizedRecommendations.js`. Hook: `client/src/hooks/usePersonalizedRecommendations.js`. Dashboard helper re-export: `client/src/pages/Dashboard/recommendationUi.js`.

- **State vocabulary** — exactly seven values: `idle` (request not started), `loading` (in progress), `ready` (`status==="ready"` and non-empty items), `no-snapshot` (`status==="no-snapshot"`), `empty` (`status==="ready"` with zero items — never converted to `no-snapshot`), `disabled` (exact checkpoint-503 disabled body only), `error` (every other failure including generic 503/500/401/400/network/payload).
- **Request** — one `GET /api/recommendations?limit=<n>` per fetch via the existing authenticated API client (`api.get`); limit integer 1..100 default 10 with no silent clamp; never sends `userId`/`user_id`/`email`/snapshot/artifact params (identity stays in the bearer token); no retry, polling, or recursive fetch.
- **Validation** — requires `success:true`, `source==="personalized-snapshot"`, `status` in `ready|no-snapshot`, array `items`; each item has positive contiguous integer `rank` 1..N in received order (no `Array.sort`/repair) and a non-empty unique `song._id`; snapshot projected to only `{snapshot_version, generated_at, item_count, available_count, unavailable_count}` with `available_count + unavailable_count === item_count`, `items.length <= available_count`, `items.length <= limit`; songs passed through un-mutated (no rank/score/fallback markers attached).
- **Errors** — sanitized `{code, message}` only (`RECOMMENDATION_REQUEST_FAILED` | `RECOMMENDATION_PAYLOAD_INVALID`); known fixed server messages may pass through, otherwise replaced with `failed to load personalized recommendations`; never surfaces raw fetch/network/config/Authorization material.
- **Hook** — `usePersonalizedRecommendations({limit=10, enabled=true})` returns `{state, songs, snapshot, error, isLoading, isReady, shouldUseFallback, refresh}`; `enabled=false` stays `idle` with zero requests; `isReady` is `state==="ready"` only; generation counter + cancellation so unmount/supersede never becomes `error`; no timers/auto-retry.
- **Fallback policy** — `shouldUseLegacyRecommendationFallback(state)` is `false` for `idle|loading|ready` and `true` for `no-snapshot|empty|disabled|error`. This is a state predicate only — **36/43 does not fetch or render any legacy/Trending/popular/random fallback data**. Empty reason helper: `none|no-snapshot|empty-snapshot|feature-disabled|request-error`.
- **Out of scope** — no manual player/`PlayHistory`/`ListeningEvent` side effects, no client-side scoring/normalization/hybrid/exploration, no new npm dependencies. Dashboard integration is **37/43** (implemented below).

### Dashboard Recommended For You (37/43)

Dashboard `client/src/pages/Dashboard/Dashboard.jsx` consumes the 36/43 hook; pure selector: `selectDashboardRecommendationPresentation` in `recommendationUi.js`.

- **Hook** — `usePersonalizedRecommendations({ limit: 10 })` only (no direct `api.get("/api/recommendations")`, no second fetcher, no polling). One initial personalized request on mount via the existing hook contract.
- **Row order** — principal content rows remain **Recently Played → Trending Now → Recommended For You**. The song search bar stays between Trending Now and Recommended For You (not moved).
- **Title** — section heading is exactly **Recommended For You** (old Dashboard title **Recommended Songs** removed; no unrelated labels renamed).
- **Presentation modes** — `loading | personalized | legacy` with mapping `idle→loading`, `loading→loading`, `ready→personalized`, `no-snapshot|empty|disabled|error→legacy`. Unknown state throws (fail closed).
- **ready** — renders only hook `songs` in server order; no legacy append/interleave/fill-to-10; no local sort/rerank/random.
- **loading / idle** — bounded placeholder (`.trending-status` + `role="status"`, message `Loading...`); **no legacy cards** (no fallback flash).
- **Fallback states** — `no-snapshot`, `empty`, `disabled`, `error` render the existing legacy source `filteredSongs` (`legacyRecommendedSongs`) exactly as before; **no extra fallback request**; not Trending/popular/random/catalog-coupled; no scary error/disabled prose required for rendering.
- **Search** — catalog search remains separate; an active search term shows existing `filteredSongs` and is not mixed into personalized ranking.
- **Playback** — card click calls `player.playSong(recommendationPresentation.songs, index)` with the **exact visible list** (no hidden queue extras; no slice unless the old UI had a cap — it did not). PlayerContext remains the sole playback/history/telemetry owner (no manual PlayHistory or ListeningEvent POST from Dashboard).
- **Not rendered** — `snapshot_version`, `generated_at`, counts, `basis`, scores, `collaborative_known`, `payload_sha256`, `artifact_version`, Mongo IDs, AI badge/confidence copy.
- **Out of scope** — no server/Python/API/model/dependency changes; no Admin metrics (**38/43**); no snapshot generation.

```bash
node --test client/src/pages/Dashboard/recommendationUi.test.js client/src/pages/Dashboard/personalizedDashboardUi.test.js
```

### Secure Admin recommendation-metrics API (38/43)

`GET /api/admin/recommendations/metrics` serves the latest immutable 32/43 evaluation run for one pipeline stage. Production router: `server/routes/adminRecommendationRoutes.js`; projection service: `server/services/adminRecommendationMetricsService.js`.

- **Auth** — existing `protect` then `adminOnly` (same middleware as `/api/admin`); no new JWT/role parsing; no `RECOMMENDATION_AI_ENABLED` / `recommendationConfig` gate (this endpoint is independent of the personalized-recommendation flag).
- **Query** — only accepted key is `pipeline_stage` with exact lowercase values `collaborative | hybrid | policy` (default `policy`). Every other key (`limit`, `userId`, `user_id`, `run_id`, `sort`, `metric`, `best`, `winner`, `artifact_version`, `foo`, …) and every non-exact value (`Collaborative`, `POLICY`, `" policy"`, empty, array, non-string) → HTTP 400 `invalid recommendation metrics query` **before** any service call (zero reads on invalid query). No history pagination, metric sorting, or best/winner selection (that surface belongs to **42/43**).
- **Read path** — service factory `createAdminRecommendationMetricsService({evaluationRunService})` calls 32/43 `listEvaluationRuns({ limit: 1, pipeline_stage })` **exactly once** (no direct `RecommendationEvaluationRun.find/aggregate/create`, no second query, no count, no cache).
- **States** — exactly `ready` or `no-runs`; source identifier is always `evaluation-history`. Empty history → HTTP 200 `{ state:"no-runs", source:"evaluation-history", pipeline_stage, latest:null }` (never 404/500, never fabricated zero metrics).
- **Ready projection (whitelist only)** — `latest` exposes exactly `{ run_id, pipeline_stage, artifact_version, evaluated_at, metrics, summary }` where `metrics` is the ten 28/43 keys and `summary` is the seven factual counts. Strips `dataset`, `configuration`, `payload_sha256`, `_id`, `createdAt`, `updatedAt`, `schema_version`; unknown extra persisted fields are ignored (not rejected, not echoed). `artifact_version` is a factual string or `null` (no artifact load/verify/activate, no “best” label). `evaluated_at` is the persisted instant (ISO-8601 string; never `createdAt`/now). Metric values pass through as finite numbers in [0,1] with **no** percent/rounding/average/overall/quality/winner labels.
- **Defensive validation** — fail-closed on missing/unsafe `run_id`, `pipeline_stage` mismatch, invalid `evaluated_at`, unsafe `artifact_version`, any missing/non-[0,1]/non-finite metric, any missing/non-integer/negative summary count, or summary invariant violations (`evaluated_user_count ≤ relevance_user_count`, `unique_recommended_at_10 ≤ catalog_size`, `diversity_evaluable_user_count ≤ evaluated_user_count`). `recommendation_user_count` **may** exceed `evaluated_user_count`. Corrupt reads are rejected; the persisted run is never mutated.
- **Errors** — `AdminRecommendationMetricsError` / `ValidationError` / `ReadError` with bounded messages only (`invalid recommendation metrics query`, `failed to load recommendation metrics`). HTTP map: 400 invalid query · 401 `protect` · 403 `adminOnly` · 200 `ready`/`no-runs` · 500 unexpected (fixed message, no `error.message`/stack/Mongo-URI leak).
- **Privacy / scope** — aggregate metrics and counts only: no user IDs/emails, Song IDs, recommendation lists, ListeningEvent/session rows, Favorite/Playlist/snapshot items, tokens, or IPs. Read-only (no create/update/delete); no Python/SVD/training/evaluation execution, no artifact publication, no client/`ml`/middleware/model/service changes for 32/43.
- **Out of scope** — Admin navigation UI (**40/43**), model-quality dashboard widgets (**41/43**), richer model-history/dataset-stats surfaces (**42/43**). Auth-matrix/access hardening tests live in **39/43** (test-only; this production section documents the endpoint contract itself).

```bash
node --test server/services/adminRecommendationMetricsService.test.js server/routes/adminRecommendationRoutes.test.js
```

### Admin metrics access regression tests (39/43)

Test-only suite for the 38/43 endpoint in `server/routes/adminRecommendationSecurity.test.js`. No production file changed (route, service, `server.js`, auth middleware, models remain byte-for-byte identical). Uses `node:test` only — fake req/res/next, injected factory, router-stack inspection, and static source checks; no Express server, no MongoDB, no new dependencies.

- **Auth short-circuits** — `protect` rejection → HTTP 401, `adminOnly`/handler/service never run; `adminOnly` rejection → HTTP 403 `Admin access required`, service never called. Query/body/header tricks (`?admin=true`, `?role=admin`, body `{role:"admin"}`, `x-admin`/`x-role`/Bearer headers) cannot bypass either gate.
- **Strict order** — middleware stack is always `protect` → `adminOnly` → handler (verified structurally and behaviorally). Auth failure precedes query parsing (no invalid-query message on 401/403).
- **Strict query** — only `pipeline_stage`; exact lowercase `policy|hybrid|collaborative`; case/whitespace/array/multi-value/non-string and every other key → HTTP 400 with **zero** service calls. Prototype keys (`constructor`, `prototype`) rejected without mutating `Object.prototype`.
- **Response minimization** — 200 payloads expose only `state`, `source`, `pipeline_stage`, `latest` (six-field whitelist); no user identity, Song/snapshot items, ListeningEvent/session rows, or `payload_sha256`/`dataset`/`configuration`/`_id`/`createdAt`/`updatedAt`/`schema_version`.
- **Error leakage** — injected synthetic secrets (Mongo URI, bearer token, user id, payload SHA, stack text) never appear in the fixed HTTP 500 body; attacker query input is never reflected in the 400 body; responses are JSON only (no HTML).
- **Read-only + one read** — static guarantee of no create/update/delete/upsert and exactly one `listEvaluationRuns({limit:1, pipeline_stage})` call site in the service; no direct `RecommendationEvaluationRun` find/aggregate; no metric sorting, winner/best/quality/composite labels.
- **No execution / no flag / no custom auth** — no Python, `child_process`, SVD, training, evaluation, ranking, artifact publish, or snapshot access in the route+service; no `RECOMMENDATION_AI_ENABLED` gate; no custom JWT/`Bearer` parsing or client-controlled role logic (authorization stays delegated to `adminOnly`); no per-user metrics filters.
- **Out of scope (not claimed)** — no penetration testing, no complete security proof, no external audit; no Admin navigation UI (**40/43**), no model-quality widgets (**41/43**), no history/dataset-stats surfaces (**42/43**).
...
### Admin evaluation history, dataset statistics, and reproducibility configuration (42/43)

Extends the 41/43 dashboard with persisted evaluation-history observability over the secure history endpoint.

- **Endpoint** — `GET /api/admin/recommendations/history` in the existing `adminRecommendationRoutes` factory; middleware order identical to `/metrics` (`protect` → `adminOnly` → handler); `server.js` mount unchanged (single `/api/admin/recommendations` mount).
- **Query** — allowed keys only `pipeline_stage` and `limit`; stage exact lowercase `collaborative|hybrid|policy`, default `policy`; limit a single decimal integer string `1..100` (default `20`) with no silent clamping; invalid input → HTTP 400 `invalid recommendation history query` before any service call.
- **Service** — `createAdminRecommendationHistoryService({evaluationRunService})` issues exactly one `listEvaluationRuns({limit, pipeline_stage})` per request (no direct `RecommendationEvaluationRun` queries); states exactly `ready|no-runs`; envelope `{state, source, pipeline_stage, limit, count, runs}` with `count = runs.length` (no database count) and server order preserved (no re-sort or quality ranking).
- **Per-run whitelist** — `run_id`, `pipeline_stage`, `artifact_version`, `evaluated_at`, ten metrics, seven summary counts + invariants, nine dataset counts + conservation, nine configuration values (missing optional configuration fields normalized to `null`); strips `payload_sha256`, `schema_version`, `_id`, `createdAt`, `updatedAt`, and any user/song/event data; stage mismatch or corrupt run fails closed.
- **Client** — `adminRecommendationHistory.js` (path constant, default limit `20`, max `100`, states `idle|loading|ready|no-runs|error`, one GET per fetch with optional AbortSignal), `useAdminRecommendationHistory` hook (generation-counter race safety; stage change clears runs then refetches; no polling), and pure UI helper `aiRecommendationHistoryUi.js` (view selector, dataset cards with the nine exact labels Raw Events / Train Events / Validation Events / Test Events / Unique Users / Unique Songs / Sessions / Interaction Pairs / Content Features, configuration cards with the nine exact labels Random Seed / Algorithm / Requested Components / Effective Components / Collaborative Weight / Content Weight / Base Hybrid Policy Weight / Explicit Profile Policy Weight / Exploration Interval, weights formatted as fixed decimals such as `0.70` with no `%`, local run selection defaulting to the first run).
- **Page** — shared single stage selector (stage change clears local run selection); **Evaluation History** row list (Evaluated At / Run ID / Artifact Version) with `aria-pressed`; **Dataset Statistics** and **Reproducibility Configuration** render only for the selected run; history Retry refreshes history only; 41/43 metrics section kept intact.
- **Not claimed** — `limit=20` is not the total history size; no trends, no “best”/winner/superior run, no retraining UI, no Python on the Admin request path, no overall/quality/composite score, no charts, no Train/Retrain/Evaluate/Activate/Deploy/Promote buttons, no second stage selector, no `RECOMMENDATION_AI_ENABLED` gate.
- **Untouched** — `server.js`, middleware, models, `recommendationEvaluationRunService`, `ml/`, Dashboard, personalized-recommendation client, `package.json` / lockfiles, `vercel.json`.

```bash
node --test server/services/adminRecommendationHistoryService.test.js \
  server/routes/adminRecommendationRoutes.test.js \
  server/routes/adminRecommendationSecurity.test.js
node --test client/src/services/adminRecommendationHistory.test.js \
  client/src/pages/Admin/aiRecommendationHistoryUi.test.js \
  client/src/pages/Admin/aiRecommendationDashboard.test.js
```

### Bounded offline retraining orchestration and Admin Model Health (43/43)

The final numbered checkpoint wires checkpoints 23–35 into **one bounded, deterministic retrain run** that is started **only from an offline CLI**, plus a read-only Admin health surface. Retraining is deliberately **not** reachable over HTTP.

```bash
node server/scripts/retrainRecommendations.js --run-id <safe-run-id> --run-at <timezone-aware-ISO8601> [--snapshot-limit <1-100>]
```

- **CLI entry (`server/scripts/retrainRecommendations.js`)** — parses only `--run-id`, `--run-at`, `--snapshot-limit` (anything else → usage error); connects through the existing `server/config/db.js` connection, calls `runRecommendationRetraining({runId, runAt, snapshotLimit})`, prints a bounded summary; exit codes **0** success, **1** retraining failure (with the failure code), **2** usage error. Run ID must match `^[a-z0-9][a-z0-9._-]{0,63}$`; run-at must be timezone-aware ISO-8601 (normalized to UTC — never server clock); snapshot limit is a strict integer `1..100` (default `20`, no silent clamp). The script is never imported by `server.js`.
- **Orchestration (`server/services/recommendationRetrainingService.js`)** — `runRecommendationRetraining()` enforces single-flight with `RecommendationRetrainingLease` (conflict → `RETRAIN_ALREADY_RUNNING`, `RETRAIN_LEASE_GRACE_MS = 60000`; the lease stores `expires_at = acquired_at + RETRAIN_PYTHON_TIMEOUT_MS + RETRAIN_LEASE_GRACE_MS` once at creation and is active iff `expires_at > now` — the grace is never re-added at read time), then records one immutable `RecommendationRetrainingAttempt` per run; identity for the whole run is `evaluation run_id = snapshot_version = artifact_version = run_id`; on success it persists the 38/43 evaluation run and up to `snapshotLimit` 35/43 per-user snapshots (concurrency 4, latest snapshot only, `created:false` counts as reused). Exactly **ten** failure codes: `RETRAIN_ALREADY_RUNNING`, `TRAINING_INPUT_LIMIT_EXCEEDED`, `INSUFFICIENT_TRAINING_DATA`, `PYTHON_TIMEOUT`, `PYTHON_FAILED`, `PYTHON_OUTPUT_INVALID`, `ARTIFACT_VERSION_CONFLICT`, `EVALUATION_PERSIST_FAILED`, `SNAPSHOT_PERSIST_FAILED`, `RETRAIN_INTERNAL_ERROR`.
- **Bounded input (`server/services/recommendationTrainingInputService.js`)** — collects ListeningEvents/Songs/Users/explicit profiles for the orchestrator under the 23/43 ceilings (`MAX_RAW_EVENTS = 250000`, `MAX_UNIQUE_USERS = 50000`, `MAX_UNIQUE_SONGS = 25000`, `MAX+1` lookahead, seed `42`, input schema version `1`); Songs or users exceeding their ceilings fail with `TRAINING_INPUT_LIMIT_EXCEEDED` (**no silent truncation**), while a `ListeningEvent` overflow keeps the newest `MAX_RAW_EVENTS` events from one `createdAt DESC, _id DESC` window (serialized ascending, flagged `event_window_truncated: true`) — never a second query, never an unflagged truncation.
- **Python runner (`server/services/recommendationPythonRunner.js`)** — the **only** server file that imports `node:child_process`; `spawn('python', ['-m', 'ml.recommender.cli', 'retrain-json'], { shell: false })` with `RETRAIN_PYTHON_TIMEOUT_MS = 600000`, stdin/stdout capped at 256 MiB and stderr at 64 KiB; no `exec`/`execSync`/`spawnSync`/`shell: true` anywhere in the server.
- **Offline pipeline (`ml/recommender/orchestrator.py` + `cli.py retrain-json`)** — reads one bounded JSON document on stdin and writes the JSON result to stdout; composes the unchanged 23–31 layers (temporal split → sparse interactions → content features → collaborative model → hybrid ranker → cold-start policy, including the fixed 0.80/0.20 and 0.70/0.30 baselines) with 28/43 evaluation against held-out relevance and 27/43 artifact publish/reuse; still offline-only (no MongoDB/HTTP from Python, no FastAPI/Flask/pymongo, no pickle) and deterministic (no wall clock, no randomness).
- **Health endpoint (`GET /api/admin/recommendations/health`)** — inside the existing admin recommendation router: `protect` → `adminOnly` → handler, **no query parameters accepted** (any key → HTTP 400 `invalid recommendation health query`); response `{success, data: {state, source, lease, latest}}` with `source` `retraining-health` and `state` exactly `running | never-run | completed | failed`; the lease is re-projected to `{active, run_id, expires_at}` (never a token or raw document) and `latest` is a fail-closed whitelist of the newest attempt (ids, status, timestamps, duration, stage, factual counts, `failure_code`/`failure_message`); read-only — no train/retrain/evaluate/activate action exists on this or any other HTTP route; HTTP 500 → fixed `failed to load retraining health`.
- **Model Health (Admin page)** — `/admin/ai-recommendation` now renders three sections: the 41/43 metrics dashboard, the 42/43 history/dataset/configuration surfaces, and the new **Model Health** section (one GET on mount + explicit **Refresh Status**, no polling/auto-retry); exact status messages (`Loading retraining health.`, `Unable to load retraining health.`, `No retraining run has completed yet.`, `A retraining run is in progress.`, `The latest retraining run completed.`, `The latest retraining run failed.`) plus lease/latest factual cards — **no** overall health score, healthy/unhealthy grade, winner/best label, retrain button, charts, or `RECOMMENDATION_AI_ENABLED` gate.
- **Not claimed / untouched** — no real production retraining run was executed for this checkpoint; `server.js`, middleware, `recommendationRoutes.js`, the personalized-recommendation client, Dashboard, `package.json`/lockfiles, and `vercel.json` are unchanged; no dependencies were added.

```bash
node --test server/models/RecommendationRetrainingLease.test.js \
  server/models/RecommendationRetrainingAttempt.test.js \
  server/services/recommendationTrainingInputService.test.js \
  server/services/recommendationPythonRunner.test.js \
  server/services/recommendationRetrainingService.test.js \
  server/scripts/retrainRecommendations.test.js \
  server/routes/adminRecommendationRoutes.test.js \
  server/routes/adminRecommendationSecurity.test.js
node --test client/src/services/adminRecommendationHealth.test.js \
  client/src/pages/Admin/aiRecommendationHealthUi.test.js \
  client/src/pages/Admin/aiRecommendationDashboard.test.js
python -m unittest discover -s ml/tests -p "test_*.py"
```
...
- **Root cause (mobile scroll)** — the base styles `html, body { overflow: hidden; height: 100% }` combined with `main { height: calc(100vh - 60px); overflow: hidden }` locked the mobile viewport: content below the fold (Trending Now, Recommended For You, Now Playing controls) could not be scrolled to, and the fixed `.controls { width: 380px }` overflowed 375/360px phones.
- **Fix (`client/src/pages/Dashboard/Dashboard.css`)** — a trailing `@media (max-width: 768px)` block makes `html, body` vertically scrollable (`overflow-x: hidden; overflow-y: auto; height: auto`), gives `main` `height: auto; min-height: calc(100vh - 60px)`, releases the stacked grids (`.scroll-grid` and its hover/collapsed variants → `height: auto; overflow: visible`), resets the negative `.scroll-grid` gutter, unclamps `.playlist-list`, and makes `.controls` `width: 100%; max-width: 380px`. Desktop rules are untouched.
- **Root cause (playback state)** — `isPlaying` was set from click intent/`loadVideo()` before the engine confirmed playback, YT init failure only advanced the track, and an error counter reset bug could skip through an entire list forever.
- **Fix (`client/src/context/PlayerContext.jsx`)** — explicit `PLAYBACK_STATUS` enum (`idle | loading | playing | paused | blocked | error`) with a `setStatus` machine; `isPlaying` flips only in `markPlaying()`, called from exactly two engine-confirmed sites (YouTube `PLAYING`, HTML audio `play`); deterministic timers — `YOUTUBE_INIT_TIMEOUT_MS = 10000` (script/init failure → `error`) and `PLAY_CONFIRM_TIMEOUT_MS = 8000` (no confirm → `blocked`, self-checked against the engine state); same-track pause/resume never reloads the media; engine-pause guards prevent a racing engine from pausing the other engine's state; `retryPlayback()` re-loads/re-plays inside a user gesture (no automatic retry loops); the error counter resets on track change and on confirmed play, and final engine errors surface as `error` instead of silent skipping.
- **UI (`Dashboard.jsx` + `playbackStatusUi.js`)** — a `role="status"` / `aria-live="polite"` surface between the progress time and controls shows the fixed literals `Loading…`, `Tap to play`, and `Unable to play this song.` with a **Retry** button (`aria-label="Retry playback"`); no raw engine error text, stack, or error code is ever rendered.
- **Unchanged** — `client/src/context/listeningTelemetry.js` (byte-identical), telemetry emission timing/semantics, section order `Recently Played → Trending Now → Recommended For You`, and every existing Dashboard/recommendation/trending contract.

```bash
node --test "client/src/**/*.test.js"   # 636 (586 baseline + 50 new)
node --test "server/**/*.test.js"       # 2036
python -m unittest discover -p "test_*.py"   # 1722 OK (3 platform skips)
python -m compileall -q ml/recommender ml/tests
python -m ml.recommender.cli runtime-info --json   # twice, byte-identical
python -m pip check
